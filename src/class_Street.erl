...
iterate_list([ Element | List ]) ->
	...
	{ Id , Length , _ , Freespeed , Count, Lanes, {}, IsCycleway, IsCyclelane } = element(2, Element),
	...
	case Lanes == 1 of
		true ->
			...
			ets:insert(list_streets, {Vertices,  Id , Length , StorageCapacity , Freespeed , Count, Lanes, {}, IsCycleway, IsCyclelane }),
	
			StorageCapacityDR = math:ceil(1 * Length / CellSizeDR ),
			ets:insert(list_streets_dr, {Vertices,  Id , Length , StorageCapacityDR , Freespeed , Count, Lanes, {}, IsCycleway, IsCyclelane });
		false ->
